...
describe Quarantine do
  ...
  context " #fetch_quarantine_list" do
    ...
    it " if dynamodb.scan fails, make sure an exception is throw" do
      allow(quarantine.database.dynamodb).to receive(:scan).and_raise(Aws::DynamoDB::Errors::LimitExceededException.new(
                                                                        Quarantine,
                                                                        "limit exceeded",
      ))
      ...
      expect(quarantine.summary[:dynamodb_failures].length).to eq(1)
      expect(quarantine.summary[:dynamodb_failures][0]).to eq("Aws::DynamoDB::Errors::LimitExceededException: limit exceeded")
    end
  end
